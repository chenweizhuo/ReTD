...
from cross_domain_class import inference_transformer
import os
...
def main(args):
    for repo_id in args.repo_ids:
        ...
        preprocessed_data_base_dir = Path("preprocessed_data/type")
        model_output_dir = output_dir / repo_id.replace('/', '_').replace('.', '') / 'GS_cross_domain' # 获得模型路径
        model_name = repo_id.replace('/', '_').replace('.', '')
        _, _, test_data_load = cross_domain_dataload(repo_id, preprocessed_data_base_dir, args.batch_size)
        model_path = model_output_dir / f"vit_model_{model_name}_GS_cross_domain.pth"
        pyramid_pooling_path = model_output_dir / f"cnn_pooling_{model_name}_GS_cross_domain.pth"
        ...
        torch.save(inference_outputs, model_output_dir / f"inference_outputs_{model_name}_GS_cross_domain.pth")
        print(f"Inference outputs saved to {model_output_dir / f'inference_outputs_{model_name}_GS_cross_domain.pth'}")
