...
def train_transformer(train_dataloader, dev_dataloader, output_dir, model_name, epochs):
    ...
    for epoch in range(epochs):
        ...
        """
        dev_accuracy, dev_precision, dev_recall, dev_f1 = evaluate(model, pyramid_pooling, dev_dataloader, device)
        print(
            f"Epoch {epoch + 1}Accuracy: {dev_accuracy * 100:.2f}%, Precision: {dev_precision * 100:.2f}%, Recall: {dev_recall * 100:.2f}%, F1: {dev_f1 * 100:.2f}%,")
        """
    model_save_path = output_dir / f"vit_model_{model_name}_only_diff.pth"
    torch.save(model.state_dict(), model_save_path)
    torch.save(pyramid_pooling.state_dict(), output_dir / f"cnn_pooling_{model_name}_only_diff.pth")
...
def main(args):
    ...
    model_output_dir = output_dir / repo_id.replace('/', '_').replace('.', '') / 'binary/only_diff'
    ...
    print(f"Inference outputs saved to {model_output_dir / f'inference_outputs_{model_name}_only_diff.txt'}")
