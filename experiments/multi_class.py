...
def train_transformer(train_dataloader, dev_dataloader, output_dir, model_name, epochs, early_stopping_patience=4):
    ...
    for epoch in range(epochs):
        ...
        dev_accuracy, dev_precision_micro, dev_recall_micro, dev_f1_micro, dev_precision_macro, dev_recall_macro, dev_f1_macro, class_accuracies = evaluate(model, pyramid_pooling, dev_dataloader, device)
        print(f"Epoch {epoch + 1} Accuracy: {dev_accuracy * 100:.2f}%")
        print(f"Micro Precision: {dev_precision_micro * 100:.2f}%, Micro Recall: {dev_recall_micro * 100:.2f}%, Micro F1: {dev_f1_micro * 100:.2f}%")
        print(f"Macro Precision: {dev_precision_macro * 100:.2f}%, Macro Recall: {dev_recall_macro * 100:.2f}%, Macro F1: {dev_f1_macro * 100:.2f}%")
        for cls_idx, cls_acc in enumerate(class_accuracies):
            print(f"Class {cls_idx} Accuracy: {cls_acc * 100:.2f}%")


    model_save_path = output_dir / f"vit_model_{model_name}_only_diff_11.pth"
    torch.save(model.state_dict(), model_save_path)
    torch.save(pyramid_pooling.state_dict(), output_dir / f"cnn_pooling_{model_name}_only_diff_11.pth")
...
def main(args):
    ...
    train_dataloader, dev_dataloader, infer_dataloader = multi_dataload(repo_id, preprocessed_data_base_dir,
                                                                        args.batch_size, i=3000)

    # 训练 Transformer 模型
    model_name = repo_id.replace('/', '_').replace('.', '')
    train_transformer(train_dataloader, dev_dataloader, model_output_dir, model_name, epochs=args.epochs)
    ...
    torch.save(inference_outputs, model_output_dir / f"inference_outputs_{model_name}_only_diff_11.pth")
    print(f"Inference outputs saved to {model_output_dir / f'inference_outputs_{model_name}_only_diff_11.pth'}")
