...
from dataloaders import cross_domain_dataload
...
def train_transformer(train_dataloader, output_dir, model_name, epochs):
    ...
    model_save_path = output_dir / f"vit_model_{model_name}_cross_domain.pth"
    torch.save(model.state_dict(), model_save_path)
    torch.save(pyramid_pooling.state_dict(), output_dir / f"cnn_pooling_{model_name}_cross_domain.pth")
...
def main(args):
    ...
    model_output_dir = output_dir / repo_id.replace('/', '_').replace('.', '') / 'cross/only_diff'
    model_output_dir.mkdir(parents=True, exist_ok=True)

    train_dataloader, infer_dataloader = cross_domain_dataload(repo_id, preprocessed_data_base_dir, args.batch_size)  # 如果需要的话，可以从train_dataset中再分出一部分作为dev_dataset

    # 训练 Transformer 模型
    model_name = repo_id.replace('/', '_').replace('.', '')
    train_transformer(train_dataloader, model_output_dir, model_name, epochs=args.epochs)

    # Perform inference
    model_path = model_output_dir / f"vit_model_{model_name}_cross_domain.pth"
    pyramid_pooling_path = model_output_dir / f"cnn_pooling_{model_name}_cross_domain.pth"
    ...
    torch.save(inference_outputs, model_output_dir / f"inference_outputs_{model_name}_cross_domain.pth")
    print(f"Inference outputs saved to {model_output_dir / f'inference_outputs_{model_name}_cross_domain.pth'}")
